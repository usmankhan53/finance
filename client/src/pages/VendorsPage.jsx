...
function VendorsPage() {
  ...
  const addOrUpdateVendor = async (e) => {
    ...
    if (newVendor.name && newVendor.contact) {
      ...
      if (isEditing) {
        // Update vendor
        try {
          const response = await fetch(`https://finance-backend-xi.vercel.app/vendor/${currentVendorName}`, {
            method: 'PUT',
            headers: {
  ...
  const deleteVendor = async (index) => {
    ...
    try {
      const response = await fetch(`https://finance-backend-xi.vercel.app/vendor/${vendorName}`, {
        method: 'DELETE',
      });
      if (response.ok) {
